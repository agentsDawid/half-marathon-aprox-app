...
def prepare_model_data(dane: RunInputData) -> dict | None:
    ...
    print(f'wiek:{age}; katW: {fill_year_category(age, dane.plec)}; Rocznik:{bornYear}')
    return {
...
with col_data:
    ...
    if userData is None:
        ...
        st.markdown(f"**Kategoria wiekowa:** {st.session_state['userdata_model']['Kategoria wiekowa'] if userData.plec else MISSING_DATA}")
